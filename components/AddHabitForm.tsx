...
export default function AddHabitForm({ onSuccess }: AddHabitFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name.trim()) {
      ...
      addHabit({
        ...
        icon,
        trackingType,
        targetCount: trackingType === "counter" ? targetCount : undefined,
  ...
  const handlePresetSelect = (preset: typeof PRESET_HABITS[0]) => {
    ...
    // Try to match a category or default to Other
    setCategory("Other");
    setColor(CATEGORIES[5].color);
    setCustomCategory("");
    setActiveTab("create");
  };
  ...
  return (
    <div className="flex flex-col h-[70vh] overflow-hidden">
        ...
        {activeTab === "create" ? (
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex justify-center">
              <div
                className="relative w-full max-w-sm p-6 rounded-3xl transition-all duration-300 shadow-xl overflow-hidden"
                style={{
              ...
              >
                ...
                <div className="mt-6 flex gap-2">
                  {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((day, i) => (
                    <div
                      key={i}
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${i < 5
      ...
      {/* Footer Action */}
      {activeTab === "create" && (
        <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex-shrink-0">
          <button
            onClick={handleSubmit}
            disabled={!name.trim()}
            className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white py-4 rounded-2xl text-base font-bold shadow-lg shadow-indigo-500/25 transition-all duration-200 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
